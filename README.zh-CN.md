...
再一个常见场景：执行命令出错了——程序跑起来报错、编译挂了，满屏错误信息。往常的做法是选中错误信息、复制、粘贴到某个 coding agent 里问"这什么意思、怎么改"。在 ruyishell 里，直接按一下空格键切到 AI 模式，问一句"这个错误是什么意思？怎么改？"就行了——ruyishell 知道你在 shell 里刚执行的命令和它的输出，上下文它自己就有，不需要拷贝粘贴。
...
M7.4（权限审批）已实现并在 Linux 验证：AI 任务执行工具前先过权限门——读类工具（read/glob/grep/ls/todo/job_*）与命中安全分类器的 bash（已知只读命令词 + 整条不含 shell 元字符/重定向）直接放行；write/edit 每次询问，未命中分类器的 bash 弹出独占一行、整行琥珀色高亮的审批条 `? 运行 <命令>（y 是 / n 否 / a 总是）`，状态行显示「等待确认」。`y` 放行本次；`n` 拒绝——拒绝文本回喂模型、任务不中断，由模型自行改道；`a` 放行并记会话级规则（bash 按命令前缀，批准 `git push` 不会连带 `git status`；文件工具按工具记忆，写密集任务只问一次）。`[agent] approval` 缺省 `ask`，可设 `always` 全自动零打扰（等价 M7.3 之前的行为）或 `never` 一律拒绝需询问操作（只读仍放行），改配置对下一个任务生效；亦可按会话用 `--always-approve`/`--never-approve` 启动标志或 `/approve`（无参数查询）切换。待审批时 `^C` 视同拒绝并终止整个任务；每次询问与决定都留痕会话日志（`approval` 记录）。详见[工作计划](docs/工作计划.md)。
...
`[agent] approval` 控制 AI 任务执行工具的权限门，三个值：`"ask"`（缺省）写操作与未列入安全分类器的命令逐次询问，`"always"` 全自动零打扰（等价无门控行为），`"never"` 对所有需询问操作一律拒绝、不弹条（拒绝文本回喂模型，由模型自行改道；只读操作仍放行）：

```toml
[agent]
approval = "ask"   # 或 "always" / "never"
```

同样的值可不改文件、按会话生效：启动时加 `--always-approve` 或 `--never-approve`（标志对该实例整个生命周期优先于 `[agent] approval`），或在 AI 模式用 `/approve`——无参数查询当前模式，`/approve ask|always|never` 设置（会话级，下个任务起生效）。
...
| `Enter`（草稿以 `/` 开头） | 提交为斜杠命令（`/model` `/approve` `/new` `/ls` `/history` `/resume` `/help` `/quit`，见「多会话」），草稿清空，仍留在 AI 模式 |
