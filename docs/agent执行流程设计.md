...
> **进度注（2026-08-31 修订）**：中间 system 的 wire 处理由"降格为带标记的独立 user 消息"改为**合并进下一条 user 消息内部**（标记行 + 上下文 + 用户原文）——"非用户消息"标记与 role=user 自相矛盾且削弱"上述"指代落地，上下文放进用户消息内部后指代与用户话语同条。共享 helper `provider.MergeContextIntoNextUser`（`internal/provider/merge_context.go`）：开头 system run 原样保留（各 provider 自己处理：OpenAI 合并为 0 位一条 system / Anthropic 进顶层 `system` 字段 / Responses 进 `instructions` 字段），中间 system 缓冲后拼进下一条 user 消息（无后续 user 的防御路径保留为独立标记 user）。三个 provider 出站形状一致。回归：compliant 单测重写（折叠/连续折叠/防御路径/全 system/clean 不变）、anthropic/responses 各新增"中间 system 进下一条 user、顶层字段不含"断言、`TestAIRequestSystemPlacementOnStrictEndpoint` 断言不变（该测试的 `t.TempDir` 清理报错为预存在环境问题，与本次改动无关，stash 复测确认）。
